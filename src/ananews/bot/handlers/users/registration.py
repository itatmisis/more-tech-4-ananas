...
from keyboards.default import registration_menu, digest_menu
...
async def source_selection(message: types.Message):
    # save role info
    answer = '2. По умолчанию вы будете получать новости из всех доступных источников\nЧтобы изменить источники ' \
...
@dp.message_handler(Text(equals=["Продолжить"]),
                    state=RegistrationState.sources)
async def time_selection(message: types.Message):
    answer = '3. По умолчанию вы будете получать новости в 12:00 по Московскому времени\nЧтобы изменить время, в ' \
...
@dp.message_handler(Text(equals=["Завершить"]),
                    state=RegistrationState.send_time)
